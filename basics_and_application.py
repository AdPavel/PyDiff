...
#     for num in file:
#         res = requests.get(f'http://numbersapi.com/{num.rstrip()}/math?json=true')
#         data = res.json()
...
# доступа к API необходимо зарегистрироваться в проекте, создать свое приложение, и получить уникальный ключ (или
# токен), и в дальнейшем все запросы к API осуществляются при помощи этого ключа.
#
...
#
# arts_dict = {}
# client_id = '0162b4bbe9a82b71d7cf'
# client_secret = 'e5e8b07c8f58a3da055a6313a48cbf79'
...
#     for id in f:
#         r = requests.get(f"https://api.artsy.net/api/artists/{id.rstrip()}", headers=headers)
#         data = r.json()
#         arts_dict[data['sortable_name']] = data['birthday']
#
# arts_dict = sorted(arts_dict.items(), key=lambda x: (x[1], x[0]))
# [print(arts) for arts, _ in arts_dict]

# ============================================ XML ===========================
# from xml.etree import ElementTree
#
# # tree = ElementTree.parse('ba/example.xml')
# # root = tree.getroot()
# #
# # # print(root)
...
# #
# # ## score summ
# # # for elem in root.iter('scores'): # findall только седи детей
# # #     score_sum = 0
# # #     for child in elem:
...
# #
# # # create tree
# # # tree.write('ba/example_copy.xml')
# #
# # ## modified tree
# #
# # greg = root[0]
...
# # print(module1, module1.text)
# # tree.write('ba/example_mod.xml')
# #
# # cer = greg[2]
# # cer.set('type', 'with distinction')
# #
# # tree.write('ba/example_mod.xml')
#
# ## add tag
# # description = ElementTree.Element('description')
# # description.text = 'Sow exelent skills'
# # greg.append(description)
# # tree.write('ba/example_mod.xml')
#
# ## remove tag
# tree = ElementTree.parse('ba/example_mod.xml')
# root = tree.getroot()
# greg = root[0]
# desc = greg.find("description")
# greg.remove(desc)
# tree.write('ba/example_mod.xml')
...
# print(res.headers['Content-Type'])
#
# par = etree.HTMLParser()
# root = etree.fromstring(res.text, par)
#
...
from xml.etree import ElementTree #import XMLParser


class MaxDepth:                     # The target object of the parser
    # maxDepth = 0
    depth = 0
    dict_color = {'blue': 0, 'red': 0, 'green': 0}

    def start(self, tag, attrib):   # Called for each opening tag.
        self.depth += 1
        # if self.depth > self.maxDepth:
        #     self.maxDepth = self.depth
        # self.list_color[self.depth - 1] += self.depth
        self.dict_color[attrib['color']] += self.depth
